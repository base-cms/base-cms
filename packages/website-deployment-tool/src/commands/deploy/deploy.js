...
module.exports = async (argv) => {
  const { site, namespace } = argv;
  const sitePath = join(process.cwd(), 'sites', site, 'package.json');
  ...
  const image = `basecmswebsites/${name}`;

  if (version !== `v${pkg.version}`) {
    log(`Site ${name} is at version ${pkg.version}. Skipping deployment.`);
    process.exit(0);
  }
  ...
  const build = async () => {
    const imageTag = `${site}:${version}`;
    log(`Building  ${image}:${version}...\n`);
    await docker(['login', '-u', DOCKER_USERNAME, '-p', DOCKER_PASSWORD]);
    await docker(['build', '-t', imageTag, '--build-arg', `SITE=${site}`, process.cwd()]);
